...
Boolean W10_560_Machine::loadExec()
{
	...
	if(not((tempname[j]>= 48 and tempname[j] <= 57) or
	       ...
	       (tempname[j]>= 97 and tempname[j] <= 122)or
	       (tempname[j]==' ')))
	{
	    processOut <<"invalid contents: HeaderRecord Error\n";
	    return false;
...
void W10_560_Machine::run()
{
	...
	object Integer tmpp;
	if(m_code_inst.getR() == 0)
	{
	    tmpp = r0.toDecimal();
	    std << "r0: ";
	}
	else if(m_code_inst.getR() == 1)
	{
	    tmpp = r1.toDecimal();
	    std << "r1: ";
	}
	else if(m_code_inst.getR() == 2)
	{
	    tmpp  = r2.toDecimal();
	    std << "r2: ";
	}
...
void W10_560_Machine::op9(Instruction &inst)
{
    ...
    object Integer count2 = 19, two_power = 1, val;
    if (reg_bin[count2] == '1')
    {
	val ++;
    }
...
void W10_560_Machine::opC(Instruction &inst) //BR R,S(X)
{
	...
	Integer tmp_r = r1.toDecimal();
	Integer tmp_s = inst.getS();
	Integer tmp_x = inst.getX();
	processTrace <<"BR 1," << tmp_s <<"(" << tmp_x <<")\n";
	...
	Integer tmp_r = r3.toDecimal();
	Integer tmp_pc = pc.toDecimal();
	Integer tmp_s = inst.getS();
	Integer tmp_x = inst.getX();
	processTrace <<"BR 3," << tmp_s <<"(" << tmp_x <<")\n";
	processTrace <<"Memory[" << s <<"]: " << val <<'\n';
